...
mod tests {
    ...
    fn test_size_aware_position_passability() {
        ...
        assert!(terrain_map.is_position_passable_for_size(center_world.0, center_world.1, 0.5, &ground_configs));
        
        // Medium pawn (size 1.0) should fit in center (with tolerance for adjacent stones)
        assert!(terrain_map.is_position_passable_for_size(center_world.0, center_world.1, 1.0, &ground_configs));
        ...
        assert!(!terrain_map.is_position_passable_for_size(center_world.0, center_world.1, 6.0, &ground_configs));
    }
    ...
    fn test_size_aware_pathfinding_success() {
        ...
        let path = terrain_map.find_path_for_size(start, goal, 2.0, &ground_configs);
    ...
    fn test_size_aware_pathfinding_blocked_by_size() {
        ...
        for x in 0..5 {
            for y in 0..3 {
                if x == 2 {
                    terrain_map.set_tile(x, y, grass_type); // Middle column passable
                } else {
                    terrain_map.set_tile(x, y, stone_type); // Sides blocked
                }
            }
        }
        ...
        let path = terrain_map.find_path_for_size(start, goal, 2.5, &ground_configs);
    ...
    fn test_size_aware_pathfinding_goal_blocked() {
        ...
        terrain_map.set_tile(5, 4, stone_type);
        terrain_map.set_tile(4, 5, stone_type);
    ...
    fn test_edge_based_collision_prevents_overlap() {
        ...
        // [G G G G G G G]
        // [G G G G G G G]
        terrain_map.set_tile(3, 1, stone_type);
        terrain_map.set_tile(3, 2, stone_type);
        terrain_map.set_tile(3, 3, stone_type);
        terrain_map.set_tile(4, 3, stone_type);
        terrain_map.set_tile(5, 3, stone_type);
        
        // Test that pawns can't be positioned on impassable tiles
        let stone_pos = terrain_map.tile_to_world_coords(3, 2);
        assert!(!terrain_map.is_position_passable_for_size(stone_pos.0, stone_pos.1, 0.5, &ground_configs));
        assert!(!terrain_map.is_position_passable_for_size(stone_pos.0, stone_pos.1, 1.0, &ground_configs));
        
        // Test that small pawns can fit in open areas away from obstacles
        let open_pos = terrain_map.tile_to_world_coords(1, 1);
        assert!(terrain_map.is_position_passable_for_size(open_pos.0, open_pos.1, 0.5, &ground_configs));
        assert!(terrain_map.is_position_passable_for_size(open_pos.0, open_pos.1, 1.0, &ground_configs));
        ...
        assert!(terrain_map.is_position_passable_for_size(diagonal_pos.0, diagonal_pos.1, 0.5, &ground_configs));
        ...
        assert!(terrain_map.is_position_passable_for_size(edge_case_pos.0, edge_case_pos.1, 0.5, &ground_configs));
    }
}