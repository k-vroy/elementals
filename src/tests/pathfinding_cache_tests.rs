...
mod tests {
    ...
    fn create_test_terrain() -> TerrainMap {
        ...
        for x in 0..10 {
            for y in 0..10 {
                if y == 2 && x >= 2 && x <= 6 {
                    terrain_map.set_tile(x, y, TerrainType::Stone); // Horizontal barrier
                } else {
    ...
    fn test_cache_basic_hit_miss_behavior() {
        ...
        let path1 = terrain_map.find_path_for_size_cached(start, goal, size, &mut cache);
        ...
        let path2 = terrain_map.find_path_for_size_cached(start, goal, size, &mut cache);
    ...
    fn test_cache_size_quantization() {
        ...
        let _path1 = terrain_map.find_path_for_size_cached(start, goal, 1.0, &mut cache);
        assert_eq!(cache.stats.path_cache_misses, 1);
        
        // Very similar size should hit cache due to quantization
        let _path2 = terrain_map.find_path_for_size_cached(start, goal, 1.01, &mut cache);
        assert_eq!(cache.stats.path_cache_hits, 1, "Similar sizes should hit cache");
        
        // Significantly different size should miss cache
        let _path3 = terrain_map.find_path_for_size_cached(start, goal, 2.0, &mut cache);
    ...
    fn test_cache_different_start_goal_combinations() {
        ...
        let _path1 = terrain_map.find_path_for_size_cached(start1, goal1, size, &mut cache);
        let _path2 = terrain_map.find_path_for_size_cached(start2, goal2, size, &mut cache);
        assert_eq!(cache.stats.path_cache_misses, 2, "Different paths should miss cache");
        
        // Repeat same combinations should hit cache
        let _path1_repeat = terrain_map.find_path_for_size_cached(start1, goal1, size, &mut cache);
        let _path2_repeat = terrain_map.find_path_for_size_cached(start2, goal2, size, &mut cache);
    ...
    fn test_passability_cache() {
        ...
        let result1 = terrain_map.find_path_for_size_cached(passable_pos, passable_pos, size, &mut cache);
        ...
        terrain_map.find_path_for_size_cached(passable_pos, passable_pos, size, &mut cache);
    ...
    fn test_terrain_change_invalidation() {
        ...
        let path3 = terrain_map.find_path_for_size_cached(start, goal, size, &mut cache);
    ...
    fn test_terrain_invalidation_basic() {
        ...
        terrain_changes.add_change(5, 5, TerrainType::Stone);
        cache.invalidate_from_terrain_changes(&terrain_changes);
    ...
    fn test_passability_cache_basic_operation() {
        ...
        let path2 = terrain_map.find_path_for_size_cached(start, goal2, size, &mut cache);
    ...
    fn test_cache_hit_ratio_calculation() {
        ...
        // First call - miss
        let _path1 = terrain_map.find_path_for_size_cached(start, goal, size, &mut cache);
        assert_eq!(cache.get_hit_ratio(), 0.0); // 0 hits / 1 total = 0%
        
        // Second call - hit
        let _path2 = terrain_map.find_path_for_size_cached(start, goal, size, &mut cache);
        assert_eq!(cache.get_hit_ratio(), 0.5); // 1 hit / 2 total = 50%
        
        // Third call - hit
        let _path3 = terrain_map.find_path_for_size_cached(start, goal, size, &mut cache);
    ...
    fn test_cache_performance_with_multiple_sizes() {
        ...
        for size in &sizes {
            let _path = terrain_map.find_path_for_size_cached(start, goal, *size, &mut cache);
        }
    ...
    fn test_performance_benchmark_cache_vs_nocache() {
        ...
        assert!(cached_duration < uncached_duration / 5, 
               "Cached pathfinding should be at least 5x faster than uncached");
            ...
            for start_y in [1, 4, 7] {
                for goal_x in [6, 8] {
                    for goal_y in [1, 4, 7] {
                        ...
                        let _path = terrain_map.find_path_for_size_cached(start, goal, 1.0, &mut cache);
                        path_count += 1;
                    }
                }
            }
    ...
    fn test_cache_with_multiple_paths() {
        ...
        for (start, goal) in &paths {
            let path = terrain_map.find_path_for_size_cached(*start, *goal, 1.0, &mut cache);
    ...
    fn test_cache_with_large_terrain_map() {
        ...
        for x in 10..40 {
            terrain_map.set_tile(x, 25, TerrainType::Stone); // Horizontal barrier
        }
        for y in 10..20 {
            terrain_map.set_tile(15, y, TerrainType::Stone); // Vertical barrier
        }
    ...
    fn test_cache_behavior_with_impossible_paths() {
        ...
        for x in 0..5 {
            terrain_map.set_tile(x, 2, TerrainType::Stone); // Complete horizontal barrier
        }
